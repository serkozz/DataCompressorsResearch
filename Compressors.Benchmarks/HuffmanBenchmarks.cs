...
— Attendez 27, — сказала Анна Павловна, соображая. — Я нынче же поговорю Lise (la femme du jeune Болконский) 28. И, может быть, это уладится. Ce sera dans votre famille que je ferai mon apprentissage de vieille fille";
        ...
        [Benchmark]
        public void Huffman_HelloWorldString()
        {
            HuffmanCompresor compressor = new();
            compressor.Build(HELLO_WORLD);
            compressor.Encode(HELLO_WORLD);
        }

        [Benchmark]
        public void Huffman_VeryLongString()
        {
            HuffmanCompresor compressor = new();
            compressor.Build(VERY_LONG_STRING);
            compressor.Encode(VERY_LONG_STRING);
        }
    }
}